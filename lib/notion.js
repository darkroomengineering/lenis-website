...
function RichTextElement({ richText }) {
  ...
  switch (type) {
    case 'text': {
      console.log('text', richText)
      const textContent = richText.text?.content || plain_text
      //   const link = richText.text?.link
      // Use top-level href if available, otherwise use link.url
      const url = richText.href

      if (url) {
        content = <Link href={url}>{textContent}</Link>
      } else {
    ...
    case 'mention': {
      ...
      if (href) {
        content = <a href={href}>{mentionContent}</a>
      } else {
        content = <span>{mentionContent}</span>
      }
      break
    }
    ...
    case 'equation': {
      ...
      if (href) {
        content = <a href={href}>{expression}</a>
      } else {
        content = <span>{expression}</span>
      }
      break
    }
    ...
    default:
      ...
      if (href) {
        content = <a href={href}>{plain_text}</a>
      } else {
        content = <span>{plain_text}</span>
      }
  }
